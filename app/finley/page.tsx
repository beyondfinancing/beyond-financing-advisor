...
const DEFAULT_LOAN_OFFICER: LoanOfficerRecord =
...
const COPY: Record<
  LanguageCode,
  {
    ...
    placeholderConversation: string;
...
> = {
  en: {
    ...
    targetScenarioText:
      "Now enter the target home price, estimated down payment, and occupancy so Finley Beyond can continue the scenario with the numbers you are actually considering.",
  ...
  pt: {
    ...
    targetScenarioText:
      "Agora informe o valor estimado do imóvel, a entrada estimada e a ocupação para que Finley Beyond continue o cenário com os números que você realmente está considerando.",
  ...
  es: {
    ...
    targetScenarioText:
      "Ahora ingrese el precio estimado de la vivienda, el pago inicial estimado y la ocupación para que Finley Beyond continúe el escenario con las cifras que realmente está considerando.",
...
Respond in ${
  ...
  const sendLeadSummary = async (trigger: SummaryTrigger): Promise<boolean> => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(extractAiText(data));
      }

      setSummaryMessage(t.summarySent);
      return true;
    } catch {
      setSummaryMessage(t.summaryError);
      return false;
    } finally {
      ...
      <style>{`
        * { box-sizing: border-box; }
        ...
        @media (max-width: 640px) {
          .bf-page-wrap { padding: 20px 14px !important; }
          .bf-card { padding: 18px !important; border-radius: 16px !important; }
          .bf-hero { padding: 22px !important; border-radius: 16px !important; }
          .bf-header-title { font-size: 24px !important; line-height: 1.2 !important; }
          .bf-section-title { font-size: 20px !important; }
          .bf-button { width: 100%; }
          .bf-chat-scroll { max-height: 340px !important; }
        }
      `}</style>
              ...
              <div className="bf-card" style={styles.card}>
                ...
                <div style={styles.actionButtonWrap}>
                  <button
                    type="button"
                    onClick={() =>
                      handleActionClick("apply", activeOfficer.applyUrl, true)
                    }
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      handleActionClick("schedule", activeOfficer.scheduleUrl, true)
                    }
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      handleActionClick("contact", mailtoHref, false)
                    }
